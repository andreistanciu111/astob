...
def today_ro_bucharest() -> str:
    # data “azi” în fusul Europe/Bucharest
    return today_ro(datetime.now(ZoneInfo("Europe/Bucharest")).date())
...
def find_col(df: pd.DataFrame, candidates: List[str]) -> str:
    ...
    for c in candidates:
        n = norm(c)
        hit = [orig for k,orig in cmap.items() if n in k]
...
def read_table_from_bytes(data: bytes) -> pd.DataFrame:
    ...
    try:
        ...
        pass
    # CSV (auto-sep și encodări frecvente RO)
...
def generate_zip_from_bytes(astob_bytes: bytes, key_bytes: bytes, template_path: str) -> bytes:
    ...
    # 2) identifică coloane (tolerant la denumiri)
    col_tid_ast  = find_col(ast, ["Nr. terminal", "TID"])
    col_sum_ast  = find_col(ast, ["Sumă tranzacție", "Suma tranzactie", "Valoare cu TVA"])
    col_prod_ast = find_col(ast, ["Nume Operator", "Nume operator", "Operator",
                                  "Denumire Produs", "Nume Comerciant", "Comerciant"])
    ...
    col_rc_key   = find_col(key, ["NR INREGISTRARE", "Nr. inregistrare R.C.", "NR. INREGISTRARE R.C.", "Nr inregistrare RC"])
    ...
    for _, r in ast.iterrows():
        ...
        if val <= 0.0:  # elimină 0
            continue
        rows_by_client.setdefault(info["client"], []).append(
            (info["site"], r["_TID"], r["_PROD"], val, r["_DT"])
        )

    # 5) pregătește șablonul
    ...
    # 6) generează câte un fișier/xlsx per client
    out_zip = BytesIO()
    with zipfile.ZipFile(out_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for client, items in rows_by_client.items():
            if not items:
                continue
            # ordonează cronologic (data + oră)
            ...
            # antet + legende
            replace_all(ws, {
                "{HEADER_DATE}": today_ro_bucharest(),
                "{COLECTARI}": colectari,
                "{NUME}": client,
            ...
            # rânduri tranzacții – copiem stilul rândului model pentru fiecare
            r = row_model
            for i, (site, tid, prod, val, dt) in enumerate(items):
                if i > 0:
                    ws.insert_rows(r)
                ...
                vcell.number_format = "General"               # curăță format vechi
                v = round(float(val), 2)
                vcell.value = v                                # NUMERIC ⇒ nu mai apare 046
                vcell.number_format = "0,00"                   # format RO „46,00”
            ...
            # total – pe rândul unde era placeholder-ul {TOTAL}, mutat după inserări
