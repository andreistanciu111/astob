...
async def generate(
    ...
    try:
        # salvează cu extensia originală
        ...
        if template is not None:
            ...
            if not template_path.exists():
                return JSONResponse({"ok": False, "error": "Template missing. Upload 'template' or add to ./static/bp model cu {}.xlsx"}, status_code=400)

        # clients zip (opțional)
        ...
        if proc.returncode != 0:
            return JSONResponse({"ok": False, "error": "Generator failed", "stderr": proc.stderr, "stdout": proc.stdout, "cmd": cmd}, status_code=500)

        if not out_zip.exists():
            listing = [str(p.relative_to(tmpdir)) for p in tmpdir.rglob("*")]
            return JSONResponse({"ok": False, "error": "ZIP not produced", "stdout": proc.stdout, "stderr": proc.stderr, "cmd": cmd, "tmp_listing": listing}, status_code=500)
