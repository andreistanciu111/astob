...
def run_generator(astob_path: Path, key_path: Path,
                  template_path: Optional[Path], clients_zip_path: Optional[Path]):
    ...
    if template_path is None:
        ...
        if not template_path.exists():
            ...
            return JSONResponse(
                {"ok": False,
                 "error": "Template missing. Upload 'template' or add to ./static/bp model cu {}.xlsx"},
                status_code=400
            )
    ...
    # Trimitem fișierul și curățăm tmp-ul DUPĂ ce răspunsul s-a livrat
    return FileResponse(
        path=str(out_zip),
...
async def generate(
    ...
    clients_zip_path: Optional[Path] = None
...
async def generate_b64(payload: dict):
    ...
    def write_b64(name_key: str, b64_key: str, fallback: str) -> Path:
        name = payload.get(name_key) or fallback
        path = tmpdir / name
        with open(path, "wb") as f:
            f.write(b64decode(payload[b64_key]))
        return path
